...
2. สร้างแท็บ **`รายละเอียดคอร์ส`** ใส่ข้อมูลคอร์ส:
...
3. สร้างแท็บ **`config`** ใส่ข้อมูลบัญชีรับโอน (แก้ไขได้ตลอดโดยไม่ต้องแตะโค้ด):
   ...
   > ระบบจะดึงข้อมูลแถว **`เลขบัญชี`** ไปตรวจสอบกับสลิปอัตโนมัติด้วย
...
2. เพิ่มทีละตัว:
   ...
   | `RECEIVER_ACCOUNT` | *(optional)* เลขบัญชีผู้รับ (ไม่มีขีด) — ใช้เป็น fallback ถ้าไม่มีแท็บ `config` |
